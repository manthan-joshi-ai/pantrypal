...
export default function WasteTracker({ stats, onReset }) {
  ...
  const lifetimeWastePct = percent(stats.leftoverItems || 0, lifetimeTotal);
  const lifetimeConsumedPct = percent(stats.ingredientsSaved || 0, lifetimeTotal);
  const periodTotal = periodTotals.used + periodTotals.leftover;
  const periodWastePct = percent(periodTotals.leftover, periodTotal);
  ...
  return (
    <div className="waste-tracker">
      <div className="waste-head">
        <div>
          <p className="waste-label">Food Waste Analytics</p>
      ...
      <div className="waste-kpis">
        <div className="waste-kpi waste-kpi--risk">
          <span className="waste-kpi-value">{lifetimeWastePct}%</span>
          <span className="waste-kpi-label">food waste</span>
          <span className="waste-kpi-note">{stats.leftoverItems || 0} of {lifetimeTotal} items</span>
        </div>
        <div className="waste-kpi waste-kpi--saved">
          <span className="waste-kpi-value">{lifetimeConsumedPct}%</span>
      ...
      <div className="waste-grid">
        ...
        <div className="waste-metric">
          <span className="waste-value">{periodWastePct}%</span>
          <span className="waste-text">{period} waste</span>
        </div>
      </div>
      ...
      <div className="waste-chart-panel">
        ...
        <div className="waste-bars" aria-label={`${period} consumption and waste chart`}>
          {chartData.map((item) => {
            ...
            return (
              <div key={item.key} className="waste-bar-col" title={`${item.label}: ${item.used} used, ${item.leftover} waste`}>
        ...
        <div className="waste-legend">
          ...
          <span><i className="legend-dot legend-dot--leftover" />Food waste</span>
        </div>
      </div>
      ...
      {stats.lastWasteRecipe && (
        <p className="waste-last">Latest waste event: {stats.lastWasteRecipe}</p>
      )}
    </div>
  );
}
