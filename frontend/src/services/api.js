...
export async function getRecommendations(ingredients, healthProfile, recipeCount = 3) {
  const res = await fetch(`${BASE}/recommend`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ ingredients, health_profile: healthProfile, recipe_count: recipeCount }),
  });
  if (!res.ok) {
